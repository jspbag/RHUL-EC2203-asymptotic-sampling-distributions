...
- Compute the **asymptotic distribution** as $n$ grows large
...
- Suppose we are interested in a random variable $Y$ with population distribution $f_Y(y)$. Suppose that 
  ...
  $$\mathrm{E}(Y)=\mu_Y; \quad \mathrm{var}(Y) = \sigma_Y^2< \infty$$
...
- We use the shorthand $\overline{Y} -\mu_Y \overset{p}{\rightarrow} 0$ and say that $\overline{Y}$ **converges in probability** to $\mu_Y$.
...
<!-- Let $\{Y_i; i=1,...,n\}$ be a sequence of i.i.d. random variables with $\mathrm{E}(Y_i) = \mu_Y$ and $\mathrm{var}(Y_i) = \sigma^2_Y$. If $\sigma^2_Y < \infty$, then as $n \rightarrow \infty$ -->

<!-- $$P(- \epsilon < \overline{Y} - \mu_Y< \epsilon) \rightarrow 1,$$ -->

<!-- for **any** $\epsilon > 0$. Write $\overline{Y} - \mu_Y \overset{p}{\rightarrow} 0$. -->

<!-- </div> -->

## The Central Limit Theorem

- The **Central Limit Theorem (CLT)** states conditions such that the distribution of $\sqrt{n}(\overline{Y} - \mu_Y)$ is very close to a $\mathcal{N}(0,\sigma_Y^2)$-distributed random variable when $n$ is large
...
- The Law of Large Numbers gives conditions such that $\overline{Y}$ is very close to $\mu_Y$ in large samples 
  ...
  Justifies replacing the unknown population mean $\mu_Y$ by the known sample average $\overline{Y}$

- The Central Limit Theorem allow us to contruct an approximation to the unknown sampling distribution of $\overline{Y}$
